...
void prewittEdge(const Mat& src, Mat& result, uchar thresh);
...
int main(int argc, char *argv[])
{
    ...
    prewittEdge(src, edge, 64);

    namedWindow("asli");
    imshow("asli", src);

    namedWindow("PrewittEdge");
...
void prewittEdge(const Mat& src, Mat& result, uchar thresh)
{
    ...
    Mat kernelX =  (Mat_<double>(3, 3) << 1, 0, -1, 1, 0, -1, 1, 0, -1);

    //inisialisasi kernel edge y
    Mat kernelY =  (Mat_<double>(3, 3) << 1, 1, 1, 0, 0, 0, -1, -1, -1);
